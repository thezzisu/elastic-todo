<template>
      ...
      <VCol>
        <VCard>
          <VCardTitle class="d-flex align-center">
            ...
            <VSpacer />
            <VBtn variant="text" color="primary" @click="chatDialog = !chatDialog">
              <VIcon>mdi-chat-processing-outline</VIcon>
            </VBtn>
          ...
          <VList>
            <VListItem
              ...
              :prepend-icon="
                +new Date(task.finishedAt) < Date.now() ? 'mdi-check' : 'mdi-checkbox-blank-outline'
              "
  ...
  <VDialog v-model="chatDialog" class="u-max-w-256">
    <ChatCard
      :source="props.source"
      :task-id="currentTaskId"
      :taskListId="props.taskListId"
...
const taskListInfo = useAsyncState(
  async () => {
    const data = await fetchMsft(props.source.data, `me/todo/lists/${props.taskListId}`)
    return data
  },
  null,
  {
...
const tasks = useAsyncState<IMsftTodoTask[]>(
  async () => {
    const data = await fetchMsft(props.source.data, `me/todo/lists/${props.taskListId}/tasks`)
    return data.value
  },
  [],
  {
...
async function onRename() {
  ...
  if (newName) {
    await fetchMsft(props.source.data, `me/todo/lists/${props.taskListId}`, {
      method: 'PATCH',
      headers: {
